...
public class BPMConfigurations {
	...
	public KieContainer kieContainer() {
		...
		final ReleaseId releaseId = ks.newReleaseId("com.dppware", "KBaseGestionExpedientes", "LATEST");
		...
		kScanner.start(50l);// Start the KieScanner polling the Maven repository in provided time
		kScanner.addListener(new KieScannerEventListener() {
			...
			public void onKieScannerUpdateResultsEvent(KieScannerUpdateResultsEvent updateResults) {
				...
				try {
					ReleaseId rel = KieServices.Factory.get().newReleaseId("com.dppware", "KBaseGestionExpedientes", "LATEST");
